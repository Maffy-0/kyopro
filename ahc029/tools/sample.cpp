...
struct Solver {
    ...
    int select_next_card(const vector<Card>& next_cards) {
        ...
        for (int i = 0; i < k; i++) {
            auto [ct, w, p] = next_cards[i];
            if (ct == CardType::WORK_SINGLE) {
                if (i == 0) F.emplace_back(1.3, i);
                else if (money < p) F.emplace_back(0.0, i);
                else F.emplace_back((double) w / (p + 0.01), i);
            } 
            if (ct == CardType::WORK_ALL) {
                if (money < p) F.emplace_back(0.0, i);
                else F.emplace_back((double) w * m / (p + 0.01), i);
            }
        }
