    ...
    for (int i = 0; i < N; i++) {
        ...
        case card::FULLPOWER:
            {
                ...
                if (ok) V[i] = {1e8+1, i, 0, card::FULLPOWER};
            }
            break;
        case card::CANCEL:
            // 最も大きい残務量をもつ
        ...
        case card::TRANSFORMATION:
            // evalの配列をつくってみる
            {
                ...
                if (cnt == M) V[i] = {10101, i, 0, card::TRANSFORMATION};
                else V[i] = {-101, i, 0, card::TRANSFORMATION};
            }
            break;
        case card::INCREASE: {
            if (rest == 20) {
...
void choose_r() {
    ...
    int costp = 1e9, idx = -1;
    for (int i = 0; i < K; i++) {
        if (tt[i] == card::INCREASE && T >= 200) {
            if (costp > P[i] && P[i] < (double) (1000 - T / 100) / 1000 * money) {
                ok = true;
                costp = P[i];
                idx = i;
            }
    ...
    for (int i = 0; i < K; i++) {
        ...
        if (tt[i] == card::TRANSFORMATION && have[card::TRANSFORMATION] == 0 && P[i] < (double) 2 * pow(2, rest)) {
            e = 1.01;
        }
    ...
    for (int i = 0; i < K; i++) {
        ...
        if ((double) P[id] <= (double) (1000 - (double)T / 100) / 1000 * money) {
            r = id;
            t[c] = tt[r];
            w[c] = ww[r];
            cout << fixed << setprecision(10) <<  "# selected card type " << ct << endl;
